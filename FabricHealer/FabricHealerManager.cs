...
namespace FabricHealer
{
    public class FabricHealerManager : IDisposable
    {
        ...
        private readonly RepairTaskHelper repairTaskHelper;
            ...
            CancellationToken token)
        {
            ...
            this.repairTaskHelper = new RepairTaskHelper(
                this.fabricClient,
                serviceContext,
                this.Token);
        ...
        public async Task StartAsync()
        {
            ...
            try
            {
                ...
                // FOR RPC impl... Do not await this. This needs to run in a long-running Task and not block.
        ...
        public async Task CancelOrResumeAllRunningFHRepairsAsync()
        {
            ...
            try
            {
                ...
                foreach (var repair in currentFHRepairTasksInProgress)
                {
                    ...
                    _ = await this.repairTaskHelper.InitializeGuanAndRunQuery(
                                foHealthData,
                                repairRules,
                                repairExecutorData);
                }
        ...
        private async Task<bool> MonitorRepairableHealthEventsAsync()
        {
            try
            {
                ...
                foreach (var evaluation in unhealthyEvaluations)
                {
                    ...
                    // This only supports the case where a replica is stuck.
            ...
            foreach (var app in supportedAppHealthStates)
            {
                    ...
                    s => s.HealthInformation.SourceId.ToLower().Contains("observer")))
                {
                    ...
                    await TelemetryUtilities.EmitTelemetryEtwHealthEventAsync(
                        ...
                        $"Application Repair Policy is enabled.",
                        Token).ConfigureAwait(false);

                    // Schedule repair task.
                    await this.repairTaskHelper.StartRepairWorkflowAsync(
                        foHealthData,
                        repairRules,
                        Token).ConfigureAwait(false);
                }
            ...
            foreach (var node in supportedNodeHealthStates)
            {
                   ...
                   s => s.HealthInformation.SourceId.ToLower().Contains("observer")))
                {
                    ...
                    await TelemetryUtilities.EmitTelemetryEtwHealthEventAsync(
                        ...
                        $"VM Repair Policy: enabled. Logic rules found: {repairRules.Count}.",
                        Token).ConfigureAwait(false);
        ...
        private async Task ProcessReplicaHealthAsync(HealthEvaluation evaluation)
        {
            ...
            foreach (var healthEvaluation in repUnhealthyEvaluations)
            {
                ...
                */
                ...
                string repairId = $"{foHealthData.PartitionId}_{foHealthData.ReplicaId}";
                foHealthData.RepairId = repairId;
                ...
                await TelemetryUtilities.EmitTelemetryEtwHealthEventAsync(
                    ...
                    $"Replica Repair Policy: enabled. Logic rules found: {repairRules.Count}.",
                    Token).ConfigureAwait(false);
